...
def gen_cost(symbolics_opt_vars,syms_opt, opt_info, o, scipyalgo):
    ...
    '''
    cmd = ["../../dptc", "noextern_precisionrand.dpt", "ip_harness.p4", "linker_config.json", "randbuild", "--symb", opt_info["symfile"]]
    ...
    if ret.returncode != 0: # stop if there's an error running interp
        ...
        quit()
    num_stg = 0
    with open('randbuild/num_stages.txt') as f:
        num_stg = int(f.readline())

    if num_stg > 12:
        return 10 # miss rate of 1000%
        #return float('inf')
